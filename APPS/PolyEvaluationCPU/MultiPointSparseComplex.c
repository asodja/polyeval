...
float complex* eval_multi_complex_poly(uint32_t n, uint32_t m, float complex* polynomial, uint32_t* exponents, float complex* xs) {
	...
	for (uint32_t j = 0; j < m; j++) {
		...
		for (uint32_t i = 0; i < n; i++) {
			x_result += (polynomial[i] * cpowf(x, exponents[i]));
		}
		result[j] = x_result;
	}
	return result;
...
{	
	...
	for (uint32_t i = 0; i < m; i++) {
		printf("n: %d, m: %d, t: %dms, r[%d]: %f+%fi\n", n, m, timer.realtime, i, creal(expected[i]), cimag(expected[i]));
	}
